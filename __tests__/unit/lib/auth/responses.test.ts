...
describe('responses', () => {
  ...
  describe('handleAuthError', () => {
    it('creates error response from AuthResult', () => {
      const event = createBaseEvent()
      const authResult = { ok: false, statusCode: 401, message: 'Invalid token' }
    ...
    it('throws error when called with success result', () => {
      ...
      expect(() => handleAuthError(event, authResult)).toThrow('handleAuthError called with success result')
    })
  })
})
